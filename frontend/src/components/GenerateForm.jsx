...
export default function GenerateForm({ onGenerated }) {
  ...
  return (
    <div className="max-w-3xl mx-auto py-12">
      <div className="mb-12">
        <h3 className="text-3xl font-bold serif mb-2">New Manuscript</h3>
        <p className="text-stone-400 text-sm">Craft your title and provide a focus to begin the writing process.</p>
      </div>

      <form onSubmit={handleGenerate} className="space-y-12">
        {/* Title Input */}
        <div className="space-y-4">
          <label className="text-[10px] font-black uppercase tracking-[0.3em] text-stone-400">1. Entry Title (Optional)</label>
          <input
            type="text"
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            placeholder="An elegant title..."
        ...
        <div className="space-y-6">
          <label className="text-[10px] font-black uppercase tracking-[0.3em] text-stone-400">2. Define your focus</label>
          <textarea
            value={description}
            onChange={(e) => setDescription(e.target.value)}
            placeholder="Describe the angle or specific details you'd like to explore..."
            rows={6}
            disabled={loading}
